...
**Pin attempt (2026-08-25) — confirmed a dead end (2026-08-26):** the SPOT app was pinned `1.5.0 → 1.5.1` (out-of-band `--trigger` publish, version `01a03649-c796-…`, `triggers[0]` `active`, still enabled) on the chance 1.5.1 fixed the dedup bug. **Zapier Product Escalations has since confirmed 1.5.1 does NOT fix it** (same dedupe field in their logs; the 1.5.1 changes don't touch this trigger) — see [⚠️ 2026-08-24](#-2026-08-24-the-swap-did-not-fix-acceptance-delivery--acceptances-still-must-be-filed-by-hand). The trigger stays on `1.5.1` (it's the latest and harmless), but the reliable fix is the **[reconciliation sweep](../spot-accepted-sweep-to-notion/)**. **Until the sweep is live (or Zapier ships a trigger fix), keep filing acceptances by the manual-replay runbook below.**
