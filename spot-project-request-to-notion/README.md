...
**Pin attempt (2026-08-25) — confirmed a dead end (2026-08-26):** the SPOT app was pinned `1.5.0 → 1.5.1` (out-of-band `--trigger` publish, version `01a03649-c796-…`, `triggers[0]` `active`, still enabled) on the chance 1.5.1 fixed the dedup bug. **Zapier Product Escalations has since confirmed 1.5.1 does NOT fix it** (same dedupe field in their logs; the 1.5.1 changes don't touch this trigger) — see [⚠️ 2026-08-24](#-2026-08-24-the-swap-did-not-fix-acceptance-delivery--acceptances-still-must-be-filed-by-hand). The trigger stays on `1.5.1` (it's the latest and harmless), and the reliable fix — the **[reconciliation sweep](../spot-accepted-sweep-to-notion/)** — is now **live (enabled 2026-08-26, hourly)**, so acceptances this trigger misses are filed automatically within the hour. The manual-replay runbook below is kept for one-offs (e.g. an acceptance you want filed immediately rather than waiting for the next sweep), but is no longer the routine path.
...
| Trigger | `App227952CLIAPI@1.5.1` / `updated_project_request`, `status: active`, `error: null` |
...
`App227952CLIAPI@1.5.1` / `updated_project_request` — **Project Request Stage Change**, "Triggers when a project request stage changes." A **polling** trigger with **no input fields**, authenticated by connection `02a5085e-1d27-853d-89b7-115a57fc4d32` ("work.flowers"). It delivers the **full record** (not a delta). Its `id` field is the composite `<request id>_<lead_stage_modified_on>`, which is why the swap expected one run per transition — but as deployed (on `@1.5.0`) it fires **only once per request** (at the first poll, stage `Pending`) and **not** on later stage changes, so it does not actually improve on the old `new_project_request` for catching acceptances. See [⚠️ 2026-08-24](#-2026-08-24-the-swap-did-not-fix-acceptance-delivery--acceptances-still-must-be-filed-by-hand). Pinned to `@1.5.1` on 2026-08-25 as a speculative fix — **untested** until the next real acceptance.
