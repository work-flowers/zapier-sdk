...
**Status:** ✅ Published and enabled. 🎉 **2026-09-14: the trigger fired automatically on a real `Pending → Accepted` transition for the first time** (Well Organised, request `02700000000006L00hE`) — the `Pending` poll at 06:04Z skipped on the sentinel, and a *second* run fired at 06:32Z carrying the accepted identity. This is the first confirmation that **SPOT `1.5.2` fixed the dedupe defect** that had made acceptances undeliverable — the whole reason for the manual-replay workaround below. **That run failed** on a workflow bug (`First Contacted` — a Contacts property that does not exist), now fixed and deployed (version `01a09ec2-…`); see [The First Contacted bug](#the-first-contacted-bug-2026-09-14). Well Organised was **re-filed by manual replay** the same day (durable run `01a09ec3-1bca-…` — reused Company `3db9…8136`, created Contact `3db9…8110` and Deal `3db9…8148`). **The [reconciliation sweep](../spot-accepted-sweep-to-notion/) backstop can now be retired** — a real acceptance has been observed to fire this trigger, the fix is live, and Well Organised is filed (the sweep shared the same `First Contacted` bug, also fixed and deployed). Retirement is Dennis's call.
...
| Version | `01a03649-c796-7f94-acf5-e1e5097d4ead` (published 2026-08-25, the 1.5.1 pin; supersedes `01a014cb-165c-…` and the earlier raced versions) |
| Visibility | **account-visible** (`is_private: false`, repo rule 8) |
| Trigger | `App227952CLIAPI@1.5.2` / `updated_project_request`, `status: active`, `error: null` (re-pinned 2026-08-26 for Zapier's dedup fix) |
...
`updated_project_request` *appeared* to dedupe on `<request id>_<lead_stage_modified_on>`, so the swap assumed it would fire **once per stage transition**, including the request's creation (its first stage event). **That assumption turned out to be wrong in production — see [⚠️ 2026-08-24](#-2026-08-24-the-swap-did-not-fix-acceptance-delivery--acceptances-still-must-be-filed-by-hand).** The composite `id` does advance in the read-action snapshot, but the deployed polling engine treats a request as already-seen after its first (`Pending`) run and never re-fires it on a later stage change. The workflow's stage gate is still correct — only `Accepted` files; `Pending`, `Declined`, `Expired`, `Secured`, `Lost`, `Abandoned` skip immediately (Declined/Expired create nothing in the CRM by decision of 2026-08-07 — see `zap.json`) — it just never receives the `Accepted` event, so acceptances are filed by manual replay until a real delivery fix ships.
...
## ⚠️ 2026-08-24: the swap did not fix acceptance delivery — acceptances still must be filed by hand

The premise of the [trigger swap](#why-the-trigger-was-swapped) — that `updated_project_request` dedupes on the composite `<request id>_<lead_stage_modified_on>` and therefore **fires a run on every stage transition** — is **empirically false as deployed**. The composite id *does* advance (visible by running the trigger as a read action), but Zapier's polling engine for this trigger deduplicates on the **underlying request**, not on that composite `id`: once a request has fired once — at creation, stage `Pending` — **no later stage change re-fires it**. That is exactly the failure mode `new_project_request` had. The "verification" behind the swap was (a) the read-action snapshot showing the id advance and (b) the Yanolja *manual replay* — neither is a trigger-fired acceptance run, and no such run has ever occurred.

**Evidence (all on the swapped version `01a014cb`):**

| Request | Pending run (at creation) | Later stage | Run for the later stage? |
| --- | --- | --- | --- |
| BMC Tooling `…003u00hE` | ✅ `01a03390`, 2026-08-24 11:38Z | → Declined 11:41Z | ❌ none — and a poll demonstrably ran at 19:28Z (it produced Commercial's Pending run), 8 h later, without firing it |
| Commercial Laundry `…003A00hE` | ✅ `01a0353e`, 2026-08-24 19:28Z | → **Accepted** 23:06Z | ❌ none, 45+ min later — filed by manual replay (below) |

The BMC row is the clincher: a poll certainly happened *after* its Declined transition (it produced Commercial's Pending run), yet BMC's changed composite id never fired. So this is not poll latency — the request is simply treated as already-seen.

This is a defect on the **Zapier/SPOT side** (the app trigger's dedup, or the durable poller), **not** in `workflow.ts`: the durable is correct and, handed an accepted payload, files exactly as designed (see the replay below). A real fix therefore cannot be a tweak to this file — it needs a different delivery mechanism (e.g. a scheduled reconciliation sweep that lists `find_project_request` and files any `Accepted`-but-unfiled request, or a serializing front door). **Scoped but deferred** (Dennis, 2026-08-24: document the workaround now, design the fix separately).

**Confirmed by Zapier Product Escalations (2026-08-26, ticket `9NPEKP-739EJ`).** They pulled the deduplication logs for this workflow's trigger subscription and verified the exact mechanism:

- **The poller dedupes on `project_request_id`, not the composite `id`.** The `updated_project_request` trigger's output-field config marks `project_request_id` as the **primary key**, and on Zapier's platform a primary output field **overrides** the root-level `id` as the dedupe key. So the composite `id` is delivered in the payload but **never consulted** for deduplication → a request can only fire once, at first sighting. Same for classic and next-gen workflows (not a workflow-type difference) — which is why our read/test-trigger always showed the advanced ids (manual reads bypass dedup entirely).
- **1.5.1 does not fix it.** Our 1.5.1 test subscription shows the same dedupe field in their logs, and 1.5.1's changes don't touch this trigger.
- **The fix is on Zapier's side** (they own SPOT); escalated to the owning team, **no timeline**. Zapier's own recommended workaround is the manual replay or a **scheduled reconciliation sweep** — which is why we built [`spot-accepted-sweep-to-notion`](../spot-accepted-sweep-to-notion/).
- **Cutover note for when a fixed trigger ships:** pin to it and **toggle the workflow off then on** so it starts a *fresh* dedupe table; pre-fix stage changes will **not** fire retroactively (the first poll of a new subscription seeds the dedupe table without triggering).

### Manual-replay runbook (until a real fix ships)

When an acceptance email arrives but no Deal appears, file it by hand. The durable is idempotent (dedupe Table + upsert-shaped, find-first writes), so a replay reuses the existing Company/Contact and writes at most one Deal:

1. **Get the accepted payload from SPOT** — `find_project_request` with `id = <request id>` (or run `updated_project_request` as a read action). Confirm `lead_stage: "Accepted"` and a real `email` (not the "accept or secure the lead" sentinel).
2. **Replay the durable with that payload as input** — CLI `zapier-sdk --experimental trigger-workflow 019fb61c-… --input '<payload>'`, or MCP `run_workflow` with `input: <payload>`. Include the full record: `project_request_id`, `email`, `first_name`, `last_name`, `company`, `website`, `lead_stage: "Accepted"`, etc. (`run_workflow` with no `input` runs an empty payload and just skips on the no-email guard — harmless, but it files nothing.)
3. **Check the run output** — `contactVia`/`companyVia` should be a *found* path (`email-table`, `mirror-table`, `contact-relation`, `notion-website`); `created` means a new record was minted, so look for a duplicate. Confirm `dealPageId` is set and `index-request` wrote the dedupe row (so a late trigger re-fire, if one ever comes, is a no-op).
...
### State left behind, and the re-file — done 2026-09-14

The failed run created the **Company** (`Well Organised`, `3db91b07-11ac-8136-…`) before dying at contact-create; **no Contact, no Deal, no dedupe row** were written (the dedupe row is written last). After the fix deployed (version `01a09ec2-…`), Well Organised was **re-filed by manual replay** (durable run `01a09ec3-1bca-…`): `companyVia: notion-website` **reused** the existing Company `3db9…8136` via the live Website query (no duplicate), created Contact `3db9…8110` (Charlie Evans) and Deal `3db9…8148` at `Lead`, indexed the email, and wrote the dedupe row (`index-request → indexed: true`) so any later re-fire or sweep is a no-op.
...
- **`updated_project_request`'s `id` is a composite** — `<project_request_id>_<lead_stage_modified_on>` — and in the read-action snapshot it advanced from `…_2026-08-06T16:26:47` to `…_2026-08-07T00:49:43`. This section originally concluded "a new id is a new run"; **that inference was wrong** — the deployed poller does not fire on the advanced id for an already-seen request (see [⚠️ 2026-08-24](#-2026-08-24-the-swap-did-not-fix-acceptance-delivery--acceptances-still-must-be-filed-by-hand)). The advance is real in the snapshot; the poll dedup ignores it.
...
`App227952CLIAPI@1.5.2` / `updated_project_request` — **Project Request Stage Change**, "Triggers when a project request stage changes." A **polling** trigger with **no input fields**, authenticated by connection `02a5085e-1d27-853d-89b7-115a57fc4d32` ("work.flowers"). It delivers the **full record** (not a delta). Its `id` field is the composite `<request id>_<lead_stage_modified_on>`, which is why the swap expected one run per transition — but as deployed (on `@1.5.0`) it fires **only once per request** (at the first poll, stage `Pending`) and **not** on later stage changes, so it does not actually improve on the old `new_project_request` for catching acceptances. See [⚠️ 2026-08-24](#-2026-08-24-the-swap-did-not-fix-acceptance-delivery--acceptances-still-must-be-filed-by-hand). Pinned `@1.5.0 → @1.5.1` on 2026-08-25 (no effect), then **re-pinned `@1.5.1 → @1.5.2` on 2026-08-26** to pick up Zapier's dedup fix (ticket `9NPEKP-739EJ`) — done via `zap.json` + the merge pipeline, not an out-of-band publish. Whether `@1.5.2` actually fires on a stage change is unverified until the next real acceptance; the daily sweep stays on as backstop until then.
...
2. `zapier-sdk --experimental publish-workflow-version 019fb61c-c04e-7783-90ba-37a70bc3415c '<source_files json>' --dependencies '<from zap.json>' --zapier-durable-version 0.10.1 --connections '{"notion_wf":{"connectionId":"02b73654-15c8-85c3-b16a-07304d2beb17"}}' --trigger '<the trigger object from zap.json, updated_project_request>'` — kebab-case flags. **`--connections` takes the publish shape `{alias:{connectionId:id}}`, not `zap.json`'s `{alias:id}`** (which the CLI rejects with `connections.notion_wf: expected object, received string` — hit on the 2026-08-25 pin); `scripts/publish-changed-zaps.mjs` does this conversion for you, so this manual form only matters for an out-of-band publish. A convenient way to pass the source verbatim from the file: `"$(jq -Rs '{"workflow.ts": .}' workflow.ts)"`.
