...
**Pin attempt (2026-08-25):** the SPOT app was pinned `1.5.0 → 1.5.1` (out-of-band `--trigger` publish, version `01a03649-c796-…`, `triggers[0]` `active`, still enabled) on the chance 1.5.1 fixed the dedup bug. This is **untested** — it can only be confirmed by the next real `Pending → Accepted` (a read/test-trigger only returns a snapshot, and there's no way to force a stage transition), and a Zapier support ticket was filed in parallel. If the next acceptance still doesn't fire a run, 1.5.1 didn't fix it and the durable reconciliation-sweep is the real fix. **Until an acceptance is observed to fire on its own, keep filing acceptances by the manual-replay runbook below.**
