// Source of truth: https://github.com/work-flowers/zapier-sdk/tree/main/scw-events-to-workflowers-block-peter
//
...
// backstop). Triggers on `event_updated` for pgao@securecodewarrior.com
// and mirrors each timed, busy, non-declined occurrence onto the
// peter@work.flowers calendar WITH ITS FULL TITLE, so SCW meetings block
...
const DEST_CALENDAR = "peter@work.flowers";
const SOURCE_CALENDAR = "pgao@securecodewarrior.com";
const DIRECTION = "scw_to_wf";
const WORKFLOW_NAME = "scw-events-to-workflowers-block-peter";

/**
 * `GCal Sync Map` — source occurrence id -> mirror event id, shared with
 * workflowers-events-to-scw-busy-peter and gcal-block-sweep-peter. Table ops are free, so
 ...
 * work.flowers calendar. workflowers-events-to-scw-busy-peter skips any event whose
 * description contains it. (Mirrors in the other direction are bare "Busy"
 * blocks with no description — matching Dennis's setup — so that direction's belt is
