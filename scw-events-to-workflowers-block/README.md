...
One half of the two-way calendar-blocking pair between Dennis's two Google Calendars. Watches **dchiuten@securecodewarrior.com** (`event_updated`, per-occurrence via `expand_recurring: true`) and mirrors every timed, busy, non-declined occurrence onto **dennis@work.flowers** **with its full title**, so SCW meetings visibly block work.flowers time. **In practice this Zap only ever sees an occurrence once** — `event_updated` does not re-fire for an occurrence it has already delivered (300 runs across this repo's three `event_updated` Zaps, 300 distinct ids; a live reschedule on 2026-09-08 that the poll returned first never produced a run), so the update/delete branches below are dead code kept as a belt and every later move, rename, decline or Free/all-day switch is propagated by [`gcal-block-sweep`](../gcal-block-sweep/)'s daily reconcile pass instead. **Cancellations do NOT reach this trigger either** — `event_updated` with `expand_recurring: true` silently drops them (proven 2026-08-31; see [`scw-cancellations-to-workflowers-unblock`](../scw-cancellations-to-workflowers-unblock/)), so deletion propagation lives in that companion Zap; the cancel branch here is kept as a belt.
...
- **Change guard**: written on the assumption that `event_updated` fires on every touch (RSVPs, description edits, Gemini notes). The run history says otherwise — no occurrence has ever fired twice — so this branch has never executed live; it stays as a belt, and [`gcal-block-sweep`](../gcal-block-sweep/) carries the same start/end/title comparison for the coming week. Confirmed as a durables bug by Zapier Product Escalations on 2026-09-08 (ticket W6ZE93-VMEWP: an extra dedupe layer keyed on the raw event id; fix filed, no ETA). When Zapier reports the fix shipped, re-verify with a live reschedule before trusting this branch again.
