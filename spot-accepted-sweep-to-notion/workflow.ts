// Source of truth: https://github.com/work-flowers/zapier-sdk/tree/main/spot-accepted-sweep-to-notion
//
// RECONCILIATION SWEEP. This is the schedule-driven backstop for
// `spot-project-request-to-notion`, whose `updated_project_request` polling
// trigger cannot deliver acceptances: Zapier's poller dedupes on the stable
// `project_request_id` (the trigger marks it the primary output field, which
// overrides the root-level composite `id` as the dedupe key), so a request
// fires once — at Pending — and never again on the transition to Accepted.
// Zapier Product Escalations confirmed this on 2026-08-26 (ticket 9NPEKP-739EJ)
// and the fix sits on their side with no timeline. Until then, this workflow
// runs on a schedule, LISTS every project request via the SPOT
// `find_project_request` search action, and files any that are Accepted but not
// yet in the shared dedupe Table — filing them with the SAME logic the trigger
// Zap uses, so a swept Deal is indistinguishable from a real-time one, and the
// shared dedupe Table makes the two safe to run side by side (if Zapier ever
// fixes the trigger, whichever files first wins and the other is a no-op).
...
//
// UNLIKE the trigger Zap, this workflow DOES call the partner tool: the SPOT
// connection is bound here as an alias (`spot`) because the sweep calls
// `find_project_request` from the workflow body. Same connection id as the
// trigger Zap's trigger auth (the work.flowers SPOT connection).
const SPOT_APP_KEY = "App227952CLIAPI";
const SPOT_CONNECTION = "spot";
...
// The schedule trigger delivers a timestamp payload this workflow ignores.
// The only input that matters is an optional `dryRun`: when true, the sweep
// lists and classifies requests and reports what it WOULD file, writing nothing
// to Notion or the dedupe Table — the safe way to exercise the main path
// against live data (`trigger-workflow … --input '{"dryRun":true}'`).
const InputSchema = z
...
async function resolveCompany(
  ...
  // 1. The mirror Table.
  if (domain) {
    const fromTable = await ctx.step(`${key}-company-via-mirror-table`, async () => {
      try {
  ...
  if (existingContactPageId) {
    const fromContact = await ctx.step(`${key}-company-via-contact`, async () => {
  ...
  if (domain) {
    const fromNotion = await ctx.step(`${key}-company-via-notion-website`, async () => {
      try {
...
async function resolveContact(
  ...
  const existing = await ctx.step(`${key}-contact-via-email-table`, async () => {
    try {
      const found = await sdk.listTableRecords({
  ...
  if (existing) {
    const filled = await ctx.step(`${key}-contact-fill-blanks`, async () => {
  ...
  if (newPageId) {
    await ctx.step(`${key}-index-contact-email`, async () => {
      try {
...
/**
 * File one accepted request, keyed by its request id so every `ctx.step` name is
 * unique within the sweep run (the run handles many requests, unlike the trigger
 * Zap's one-per-run). Mirrors the trigger Zap's per-request pipeline exactly —
 * dedupe -> company -> contact -> deal -> index -> comment — so a swept Deal is
 * indistinguishable from a real-time one.
 *
 * `dryRun` short-circuits right after the (read-only) dedupe check: it reports
 * what it WOULD file and performs no create/patch/table write.
 */
async function fileRequest(
  ...
  // Already filed? The dedupe Table is SHARED with spot-project-request-to-notion,
  // so a request either Zap has filed is a no-op here. A failed read rethrows —
  // a false "not seen" is exactly how a duplicate Deal gets minted.
  const seen = await ctx.step(`${key}-dedupe-request`, async () => {
    try {
      const found = await sdk.listTableRecords({
        table: REQUEST_TABLE,
        keyMode: "names",
        filters: [
          { fieldKey: "Request Id", operator: "exact", value: req.requestId },
        ],
        pageSize: 1,
      });
  ...
  if (seen) {
    return { requestId: req.requestId, status: "already-filed", ...seen };
  }
  ...
  // Company first, so the Contact can be created already linked to it. A
  // consumer mailbox yields no domain (the host is the provider's, not the
  // employer's), so a Gmail requester is not filed under a company called Gmail.
  const emailDomain = isFreemail(req.email) ? "" : normalizeDomain(req.email);
  const domain = normalizeDomain(req.website) || emailDomain;

  const knownContactPageId = await ctx.step(`${key}-peek-contact`, async () => {
    try {
      const found = await sdk.listTableRecords({
        table: CONTACT_EMAIL_TABLE,
        keyMode: "names",
  ...
  const company = await resolveCompany(ctx, key, req, domain, knownContactPageId);

  // A client id links the company into the referral-lead pipeline. Created
  // companies get it in their create props; an existing company is only filled
  // where empty. Best-effort — a failure here must not cost the Deal.
  if (req.clientId && company.pageId && company.via !== "created") {
    await ctx.step(`${key}-fill-company-client-id`, async () => {
      try {
  ...
  const contact = await resolveContact(ctx, key, req, company.pageId);
  ...
  if (!dealPageId) {
    throw new Error(`Deal creation returned no page id for ${req.requestId}`);
  }

  // Index LAST: a row here means "this request is filed", and it must not be
  // able to say that before the Deal exists.
  await ctx.step(`${key}-index-request`, async () => {
    try {
      await sdk.createTableRecords({
        table: REQUEST_TABLE,
        keyMode: "names",
  ...
  if (company.via === "created" || company.via === "contact-relation") {
    ...
    await addComment(
      ctx,
      `${key}-comment-inference`,
      dealPageId,
      `${how}. Zapier Partner Directory project request \`${req.requestId}\` ` +
        `(filed by the reconciliation sweep). Deal opened at Lead — Type, Value ` +
        `and Expected Close are deliberately empty for whoever scopes it.`,
    );
  }
...
const workflow = defineDurable<Record<string, unknown>, unknown>(
  ...
  async (ctx, rawInput) => {
    ...
    const dryRun = Boolean((input as { dryRun?: boolean }).dryRun);

    // The dedupe store is what makes this workflow safe to retry AND safe to run
    // beside the trigger Zap. Without it, refuse — see REQUEST_TABLE.
    if (!REQUEST_TABLE) {
      return { skipped: true, reason: "REQUEST_TABLE is not configured" };
    }

    // List every project request. `find_project_request` with no filters returns
    // the account's full set ("leave both blank to return every project
    // request"). One search action = one task per run; see README on cadence.
    const listed = await ctx.step("list-project-requests", async () =>
      sdk.runAction({
        appKey: SPOT_APP_KEY,
        actionType: "search",
    ...
    const payload = listed as { data?: unknown; results?: unknown };
    const rawRequests: unknown[] = Array.isArray(payload?.data)
      ? (payload.data as unknown[])
      : Array.isArray(payload?.results)
        ? (payload.results as unknown[])
        : [];

    // Keep only Accepted requests carrying a usable email — the same two gates
    // the trigger Zap applies (Declined/Expired/Pending create nothing by
    // policy; no email means no contact to key on). Dedupe by requestId within
    // the batch too, so a doubled list row can't double-file in one run.
    const seenIds = new Set<string>();
    const toConsider: RequestData[] = [];
    for (const r of rawRequests) {
      const req = extractRequest(r);
      if (req.stage && req.stage.toLowerCase() !== "accepted") continue;
      if (!req.email) continue;
      if (!req.requestId || seenIds.has(req.requestId)) continue;
      seenIds.add(req.requestId);
      toConsider.push(req);
    }

    console.log(
      `Sweep: ${rawRequests.length} requests listed, ${toConsider.length} ` +
        `Accepted with an email${dryRun ? " (dryRun — nothing will be written)" : ""}`,
    );

    // Each request is independent; the shared dedupe Table keeps re-runs and the
    // trigger Zap idempotent. Fan out — one keyed set of steps per request.
    const results = await Promise.all(
      toConsider.map((req) => fileRequest(ctx, req, dryRun)),
    );
    ...
    return {
      dryRun,
      checked: rawRequests.length,
      acceptedWithEmail: toConsider.length,
      filedCount: count("filed"),
      alreadyFiledCount: count("already-filed"),
      wouldFileCount: count("would-file"),
      results,
    };
  },
);
