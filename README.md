...
| --- | --- | --- | --- |
| [`merge-duplicate-contacts`](merge-duplicate-contacts/) | Durable | ⚠️ Published, trigger not wired | Merges a Contact into the record its `Duplicate of` points at — but only when identity is corroborated (equivalent names or the same LinkedIn profile; **differing LinkedIn URLs are a hard decline**). Fills empty target fields, ORs checkboxes, **unions** every multi-value property, never copies the `Duplicate of` / `Possible duplicate` relations, and **never deletes anything**. Replaces the "Contact Merger" Notion Custom Agent, disabled 2026-07-28. Needs a Notion DB automation on Contacts (`Duplicate of` edited) pointed at its catch URL before it can fire. |
