...
| [`start-a-timer-from-notion-task`](start-a-timer-from-notion-task/) | Durable | ✅ Enabled (cutover pending) | Notion Tasks "Track Time" button → start a Harvest timer on the project mapped to the task's Notion Project; a second click the same day restarts the existing entry instead of duplicating it. Migrated from the classic Zap of the same name via the Durables UI — the migration output was non-functional (didn't parse; four Zapier template expressions left as literal strings; two step outputs mis-bound to `input`; trigger unclaimed) and was rewritten before publishing. **The Notion Tasks DB automation still has to be pointed at the catch URL** (`zap.json` → `trigger.webhook_url`), and one row in the Harvest Projects table needs fixing — see the directory README. |
