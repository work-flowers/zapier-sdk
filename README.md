...
| [`email-db-updates`](email-db-updates/) | Durable | ⚠️ Not deployed | Notion Emails page added → poll for the mail block (via Notion MCP; not exposed by the public API) → parse From/To/Cc/MessageId → resolve Contacts through the email → page-id Zapier Table (AI-classify unknowns, create individuals with the Contacts default template) → patch the page (Contacts merged, Comment Access overwritten). Migration of the `notion-worker-email-db-updates` Notion Worker; source ready, awaiting first publish + webhook cutover. |
