...
- **Two live Zaps still pass `WORKFLOW_NAME` (a constant) as `defineDurable`'s name and will fail the analyzer's `missing-run-function` rule at their next republish:** `scw-events-to-workflowers-block` and `workflowers-events-to-scw-busy`. Inline the literal (as their `-peter` copies do since 2026-09-25) in the same PR as whatever change next touches them — a republish of either will not go through otherwise.
- **PR #176's publish (2026-09-25) created `scw-events-to-workflowers-block-peter`'s container and then failed its v1 publish**, leaving an orphan workflow with no version whose id was never logged (the publisher now logs it). Peter's login cannot see CI-created workflows, so someone on the CI identity should `list-workflows`, find the duplicate `scw-events-to-workflowers-block-peter` with `current_version_id: null`, and `delete-workflow` it.
