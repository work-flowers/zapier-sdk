...
- **`missing-run-function` — `defineDurable`'s NAME argument must be a string literal.** Pass it as a constant (`const WORKFLOW_NAME = "x"; defineDurable(WORKFLOW_NAME, async (ctx, input) => …)`) and the analyzer loses track of the handler and refuses with `defineDurable must pass an async (ctx, input) => { ... } handler`, pointing at the `defineDurable(` line. Same call with `"x"` inline passes; the generics (`defineDurable<Input, unknown>`) are not the problem. First seen 2026-09-25, on a Zap whose sibling with an inline literal had published minutes earlier in the same run. If the name is also used elsewhere (a mirror description, a marker), keep the constant for those uses and repeat the literal at the call.
