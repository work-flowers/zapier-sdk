...
| Reconcile pass (2026-09-08, this repo) | Type-checked offline (`tsc --strict --noUnusedLocals`), body scanned for template-literal step ids. **Not yet exercised live** — the cloud session had no CLI credentials for `run-durable`. First check after publish: trigger a manual `{"reconcile_days":7,"dryRun":true}` run and confirm `reconciled.rowsChecked` is non-zero and `orphans` is empty (the four AI COE orphans were removed by hand on 2026-09-08). |
