...
| Update / unmirror / revive paths (2026-09-08, this repo) | Type-checked offline (`tsc --strict --noUnusedLocals`), body scanned for template-literal step ids. **Not yet exercised live.** First check after publish: `{"reconcile_days":7,"dryRun":true}` and inspect `reconciled.updated` / `reconciled.unmirrored` / `revived` per direction; a moved occurrence should appear under `updated` with its new times, and nothing that is still at its recorded time should. |
