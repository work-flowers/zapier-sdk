...
| Field | Meaning |
| --- | --- |
| `from_days` / `to_days` | Window override in days from now. **Backfill at cutover: `{"from_days":0,"to_days":30}`.** |
...
- Steady-state cost: 2 `event_v2` searches/day (1 task each) + 1 task per occurrence newly entering the horizon. A full 30-day backfill is ~9 searches + 1 task per mirror created.
